...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    // Always reads ALL integration tokens fresh from DB — never cached in JWT.
    // This means any newly-connected integration appears immediately without logout.
      ...
      if (session.userId) {
        try {
          const integrations = await prisma.integration.findMany({
            ...
            select: { provider: true, accessToken: true, teamName: true, metadata: true },
          });
          ...
          for (const i of integrations) {
            switch (i.provider) {
              ...
              case "zoom":
                ...
                break;
              case "notion":
                session.notionAccessToken = i.accessToken;
                session.hasNotion = true;
                session.notionWorkspace = i.teamName ?? undefined;
                break;
              case "github":
                session.githubAccessToken = i.accessToken;
                session.hasGithub = true;
                try {
                  const meta = i.metadata ? JSON.parse(i.metadata) as Record<string, unknown> : {};
                  session.githubUsername = (meta.username as string) ?? i.teamName ?? undefined;
                } catch { session.githubUsername = i.teamName ?? undefined; }
                break;
              case "todoist":
          ...
          // Explicit false for missing integrations (used in UI)
          ...
          if (!integrations.find((i) => i.provider === "notion")) session.hasNotion = false;
