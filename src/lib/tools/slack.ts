...
function slackError(toolName: string, err: unknown): string {
    ...
    const friendly: Record<string, string> = {
        ...
        missing_scope:
            "Bot is missing required permissions. Re-install the Slack app with chat:write and channels:read scopes.",
    };

    const slackCode = (err as { code?: string; data?: { error?: string } })
        ?.data?.error;
    ...
    },
    {
        ...
        schema: z.object({
            channel: z
                .string()
                .describe(
                    "Channel name (e.g. general, random) or Slack channel ID (e.g. C0123456)",
                ),
    ...
    },
    {
        ...
        schema: z.object({
            ...
            meetingLink: z
                .string()
                .optional()
                .describe("URL to join the meeting"),
