...
function getClient(config?: RunnableConfig): Client {
  ...
  if (!token) {
    throw new Error("Notion is not connected. Please go to Dashboard → Connect Notion.");
  }
...
function notionError(toolName: string, err: unknown): string {
  ...
  const friendly: Record<string, string> = {
    "unauthorized": "Notion token is invalid or expired. Please reconnect Notion from the Dashboard.",
    "restricted_resource": "You don't have access to this Notion resource. Make sure you shared it with your integration.",
    "object_not_found": "Notion page or database not found. Make sure it's shared with your integration.",
    "validation_error": "Invalid data format for Notion API.",
  };
  for (const [key, val] of Object.entries(friendly)) {
    if (msg.toLowerCase().includes(key)) return JSON.stringify({ error: val });
  }
  ...
  async ({ query }: { query: string }, config?: RunnableConfig) => {
    try {
      ...
      const results = res.results.map((r) => {
        ...
        // Get title — works for both pages and database entries
...
export const getNotionPageTool = tool(
  ...
  },
  {
    ...
    schema: z.object({
      pageId: z.string().describe("Notion page ID from search_notion results"),
    }),
  }
);
...
export const createNotionPageTool = tool(
  async (
    { title, content, parentPageId, databaseId }: {
      title: string;
      content: string;
      parentPageId?: string;
      databaseId?: string;
    ...
    try {
      ...
      // Build parent — either a page or a database
      ...
      const contentBlocks = content
        ...
        .slice(0, 50) // Notion limit
        .map((line) => {
          if (line.startsWith("# ")) return {
            type: "heading_1" as const,
            heading_1: { rich_text: [{ type: "text" as const, text: { content: line.slice(2) } }] },
          };
          if (line.startsWith("## ")) return {
            type: "heading_2" as const,
            heading_2: { rich_text: [{ type: "text" as const, text: { content: line.slice(3) } }] },
          };
          if (line.startsWith("• ") || line.startsWith("- ")) return {
            type: "bulleted_list_item" as const,
            bulleted_list_item: { rich_text: [{ type: "text" as const, text: { content: line.slice(2) } }] },
          };
          return {
      ...
      const page = await notion.pages.create({
        ...
        children: contentBlocks as Parameters<typeof notion.pages.create>[0]["children"],
      });
  ...
  },
  {
    ...
    schema: z.object({
      ...
      content: z.string().describe("Page content. Use markdown-like formatting: # for h1, ## for h2, • for bullets"),
      parentPageId: z.string().optional().describe("ID of parent page to nest under (from search_notion)"),
      databaseId: z.string().optional().describe("Database ID to add as a database record"),
    }),
  }
);
...
export const appendToNotionPageTool = tool(
  ...
  },
  {
    ...
    schema: z.object({
      pageId: z.string().describe("Notion page ID (from search_notion or create_notion_page)"),
  ...
  async (_: object, config?: RunnableConfig) => {
    try {
      ...
      const databases = res.results.map((r) => {
        ...
        return { id: db.id, title, url: db.url, lastEdited: db.last_edited_time };
      });
