...
function getKey(): string {
  ...
  if (!key) throw new Error("Web search is not configured. Add TAVILY_API_KEY to .env.local (free at app.tavily.com).");
  return key;
}
...
export const webSearchTool = tool(
  async (
    { query, depth, maxResults }: { query: string; depth?: string; maxResults?: number }
  ) => {
    try {
  ...
  {
    name: "web_search",
    description: "Search the web for any current information, facts, or research. Use when user asks about recent events, news, facts you might not know, technical documentation, or anything requiring up-to-date information. Returns an AI-synthesised answer plus source links.",
    schema: z.object({
      query: z.string().describe("Search query — be specific for best results"),
      depth: z.enum(["basic", "deep"]).default("basic").describe("Use 'deep' for thorough research, 'basic' for quick lookups"),
...
export const searchNewsTool = tool(
  async (
    { query, days, maxResults }: { query: string; days?: number; maxResults?: number }
  ) => {
    try {
      const data = await tavilySearch({
        query,
  ...
  {
    name: "search_news",
    description: "Search for recent news articles on any topic. Use when user asks about current events, latest news, what happened recently, breaking news, or anything time-sensitive.",
    schema: z.object({
