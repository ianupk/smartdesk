...
export async function GET(req: NextRequest) {
    ...
    // User denied access
    if (error) {
        return NextResponse.redirect(new URL(`/dashboard?todoist_error=${encodeURIComponent(error)}`, req.url));
    }

    if (!code || !state) {
        return NextResponse.redirect(new URL("/dashboard?todoist_error=invalid_callback", req.url));
    }
    ...
    try {
        ...
        return NextResponse.redirect(new URL("/dashboard?todoist_error=invalid_state", req.url));
    }
    ...
    try {
        ...
        if (!tokenRes.ok) {
            ...
            return NextResponse.redirect(new URL("/dashboard?todoist_error=token_exchange_failed", req.url));
        }
        ...
        if (!tokens.access_token) {
            ...
            return NextResponse.redirect(new URL("/dashboard?todoist_error=no_token", req.url));
        }
        ...
        try {
            ...
            if (profileRes.ok) {
                ...
                fullName = (profile.full_name as string) ?? (profile.name as string) ?? "Todoist User";
            }
        } catch {
        ...
        return NextResponse.redirect(
            new URL(`/dashboard?todoist_success=1&name=${encodeURIComponent(fullName)}`, req.url),
        );
    } catch (err) {
        console.error("[todoist-callback] error:", err);
        return NextResponse.redirect(
            new URL(`/dashboard?todoist_error=${encodeURIComponent((err as Error).message)}`, req.url),
        );
    }
}
