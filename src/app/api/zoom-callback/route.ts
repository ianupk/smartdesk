...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const displayName = profile.first_name ? `${profile.first_name} ${profile.last_name ?? ""}`.trim() : (profile.display_name ?? "Zoom Account");

        await prisma.integration.upsert({
            where: { userId_provider: { userId, provider: "zoom" } },
            create: {
                userId, provider: "zoom",
        ...
        return NextResponse.redirect(new URL(`/dashboard?zoom_success=1&name=${encodeURIComponent(displayName)}`, req.url));
    } catch (err) {
        console.error("[zoom-callback] error:", err);
        return NextResponse.redirect(new URL(`/dashboard?zoom_error=${encodeURIComponent((err as Error).message)}`, req.url));
    }
}
