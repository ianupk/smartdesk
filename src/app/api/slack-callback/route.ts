...
export async function GET(req: NextRequest) {
    ...
    // User denied access
    if (error) {
        return NextResponse.redirect(new URL(`/dashboard?slack_error=${encodeURIComponent(error)}`, req.url));
    }

    if (!code || !state) {
        return NextResponse.redirect(new URL("/dashboard?slack_error=invalid_callback", req.url));
    }
    ...
    try {
        ...
        return NextResponse.redirect(new URL("/dashboard?slack_error=invalid_state", req.url));
    }
    ...
    try {
        ...
        return NextResponse.redirect(
            new URL(`/dashboard?slack_success=1&team=${encodeURIComponent(teamName)}`, req.url),
        );
    } catch (err) {
        console.error("[slack-callback] error:", err);
        return NextResponse.redirect(
            new URL(`/dashboard?slack_error=${encodeURIComponent((err as Error).message)}`, req.url),
        );
    }
}
