...
const ZoomLogo = () => (
    <svg viewBox="0 0 24 24" className="w-5 h-5">
        ...
        <path
            d="M14.5 8H5a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2h9.5a1.5 1.5 0 0 0 1.5-1.5v-5A1.5 1.5 0 0 0 14.5 8zm4.146 1.269L16 11v2l2.646 1.731A.75.75 0 0 0 20 14.1V9.9a.75.75 0 0 0-1.354-.631z"
            fill="#fff"
        />
    </svg>
);
...
const INT_DEFS = [
    ...
    },
    {
        ...
        onDisconnect: () => {
            if (confirm("Disconnect Zoom?")) window.location.href = "/api/zoom-connect?action=disconnect";
        },
    },
    {
        id: "github",
        name: "GitHub",
        sub: "Code & repos",
        sessionKey: "hasGithub",
        Logo: GitHubLogo,
        onConnect: () => {
            window.location.href = "/api/github-connect?action=connect";
        },
        onDisconnect: () => {
            if (confirm("Disconnect GitHub?")) window.location.href = "/api/github-connect?action=disconnect";
        },
    },
    {
        id: "todoist",
        name: "Todoist",
        sub: "Tasks & projects",
        sessionKey: "hasTodoist",
        Logo: TodoistLogo,
        onConnect: () => {
            window.location.href = "/api/todoist-connect?action=connect";
        },
        onDisconnect: () => {
            if (confirm("Disconnect Todoist?")) window.location.href = "/api/todoist-connect?action=disconnect";
        },
    },
];

// Unified Connected/Disconnect pill button component
function IntStatusBtn({
    isConnected,
    onConnect,
    onDisconnect,
}: {
            ...
            <button
                ...
                onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.boxShadow =
                        "0 0 0 3px var(--accent-glow), 0 2px 12px var(--accent-glow)";
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 md:mb-10">
                        ...
                        >
                            ...
                            <span className="hidden sm:flex items-center gap-0.5 select-none">
                                <span
                                    className="inline-flex items-center justify-center rounded px-1 py-0.5 text-[10px] font-medium leading-none"
                                    style={{
                                        background: "var(--bg-2)",
                                        border: "1px solid var(--border)",
                                        color: "var(--text-3)",
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                {STATS.map((stat) => (
                                    ...
                                    >
                                        ...
                                        <div
                                            className="shrink-0"
                                            style={{ color: stat.accent ? "var(--accent)" : "var(--text-3)" }}
                                        >
                                            {stat.icon}
                                        </div>
                                        {/* Value + label */}
                                        <div className="sm:mt-4">
                                            <p
                                                className="text-2xl md:text-[26px] font-bold leading-none tracking-tight"
                                                style={{
                                        ...
                                        {stats.recentThreads.map((t, idx) => (
                                            <Link
                                                ...
                                                style={{
                                                    background: "var(--bg-1)",
                                                    borderBottom:
                                                        idx < stats.recentThreads.length - 1
                                                            ? "1px solid var(--border-soft)"
                                                            : "none",
                                                }}
                                                onMouseEnter={(e) =>
