...
function ProfilePopup({ name, email, onClose, onLogout }: {
            ...
            <div className="w-full max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between">
                ...
                <Link href="/dashboard" className="flex items-center gap-2.5 group">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
                        style={{ background: "linear-gradient(135deg, var(--accent), #f59e0b)", boxShadow: "0 2px 8px var(--accent-glow)" }}>
                        <AppIcon />
                    </div>
                    <span className="font-semibold text-sm tracking-tight" style={{ color: "var(--text)" }}>SmartDesk</span>
                    <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded"
                        style={{ background: "var(--accent-2)", color: "var(--accent)", border: "1px solid rgba(217,119,6,0.22)" }}>
                        AI
                    </span>
                </Link>
                ...
                <div className="relative">
                    ...
                    {open && (
                        <ProfilePopup
                            ...
                            onLogout={() => { setOpen(false); signOut({ callbackUrl: "/login" }); }}
                        />
                    )}
                </div>
            </div>
